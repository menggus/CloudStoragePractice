...
-- tabuserfile
CREATE TABLE `tabuserfile`
(
    `id`          int(11) NOT NULL PRIMARY KEY AUTO_INCREMENT,
    `user_name`   varchar(64)   NOT NULL COMMENT '用户',
    `file_sha1`   char(64)      NOT NULL DEFAULT '' COMMENT '文件hash值',
    `file_name`   varchar(256)  NOT NULL DEFAULT '' COMMENT '文件名',
    `file_size`   bigint(20) DEFAULT '0' COMMENT '文件大小',
    `upload_at`   datetime               DEFAULT NOW() COMMENT '创建日期',
    `last_update` datetime               DEFAULT NOW() ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `status`      int(11) NOT NULL DEFAULT '0' COMMENT '状态（可用/禁用/已删除）',
